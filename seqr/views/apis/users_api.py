...
from seqr.views.utils.orm_to_json_utils import (
    get_json_for_user,
    get_json_for_project_collaborator_list,
    get_project_collaborators_by_username,
    get_json_for_project_collaborator_groups,
    PROJECT_ACCESS_GROUP_NAMES,
)
...
require_anvil_disabled = active_user_has_policies_and_passes_test(
    lambda u: not anvil_enabled()
)
...
def get_all_collaborator_options(request):
    ...
    return create_json_response(
        {
            user.username: get_json_for_user(
...
def get_all_user_group_options(request):
    ...
    return create_json_response(
        {"groups": sorted(groups.values_list("name", flat=True))}
    )
...
def forgot_password(request):
    ...
    if users.count() != 1:
        return create_json_response(
            {}, status=400, reason="No account found for this email"
        )
    user = users.first()
...
def update_user(request):
    ...
    return create_json_response(
        get_json_for_user(request.user, {"first_name", "last_name", "display_name"})
    )
...
def create_project_collaborator(request, project_guid):
    project = get_project_and_check_permissions(
        project_guid, request.user, can_edit=True
    )
    ...
    return create_json_response(
        {
            "projectsByGuid": {
                project_guid: {
                    "collaborators": get_json_for_project_collaborator_list(
    ...
    return create_json_response(
        {
            "projectsByGuid": {
                project.guid: {
                    "collaboratorGroups": get_json_for_project_collaborator_groups(
                        project
                    )
                }
            }
